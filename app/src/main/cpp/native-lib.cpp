...
Java_com_example_user_ndktest_MainActivity_ndkEmboss(
        JNIEnv* env, jobject thiz,
                jintArray data, jint width, jint height) {
            ...
            for (y = 0; y <= height - 1; y++) {
                for (x = 0; x <= width - 1; x++) {
                    ...
                    i7 = yr * width + x;
                    i8 = yl * width + x;
                    Nx = tem[i1] + tem[i5] + tem[i3] - tem[i4] - tem[i6] - tem[i2];
                    Ny = tem[i3] + tem[i7] + tem[i2] - tem[i1] - tem[i8] - tem[i4];
